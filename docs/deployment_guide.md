...
### Key Pair (Required for Ansible SSH Access)

You must create an EC2 Key Pair in your target region (default `us-east-1`) for Ansible to connect to instances.

**Quick Setup:**

```bash
# Generate SSH key pair locally
ssh-keygen -t rsa -b 4096 -f ~/.ssh/devops-demo-key -C "devops-demo"

# Import to AWS (replace region if needed)
...
For detailed instructions and alternative methods, see [SSH Setup Guide](./ssh_setup_guide.md).

**Important:** The key name must match the `key_name` variable in `infra/terraform/variables.tf` (default: `devops-demo-key`).
